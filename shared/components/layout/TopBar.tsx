...
interface TopBarProps {
  user?: TopBarUser;
  /** Optional path to the full notifications page for this role */
  notificationsHref?: string;
}

export function TopBar({ user, notificationsHref = '/notifications' }: TopBarProps) {
  ...
  useEffect(() => {
    if (!showNotifications || !accessToken) return;
    void fetchNotifications(accessToken);
  }, [showNotifications, accessToken, fetchNotifications]);
                ...
                {isLoading && preview.length === 0 ? (
                  ...
                  preview.map((item) => (
                    <div
                      key={item.id}
                      className={cn(
                        'px-4 py-2.5 hover:bg-neutral-50 cursor-default transition-colors',
                  ...
                  <p className="px-4 py-6 text-center text-xs text-neutral-400 font-medium">
                    Chưa có thông báo nào
                  </p>
                )}
              </div>
              <div className="px-4 pt-2 border-t border-neutral-50">
                <Link
                  href={notificationsHref}
                  onClick={() => setShowNotifications(false)}
                  className="block text-center text-[11px] font-bold text-brand-500 hover:underline py-1"
                >
                  Xem tất cả
                </Link>
              </div>
          ...
          {showProfileMenu ? (
            <div className="absolute right-0 mt-3 w-48 bg-white rounded-[32px] border border-neutral-100 shadow-xl z-50 py-2.5 animate-in fade-in-0 zoom-in-95 duration-150">
              ...
              <div className="py-1">
                <button
                  type="button"
                  className="flex items-center gap-2 w-full px-4 py-2 text-xs text-neutral-600 hover:text-brand-500 hover:bg-brand-50/40 transition-colors font-semibold"
                >
                  <Settings className="w-3.5 h-3.5" /> Thiết lập tài khoản
                </button>
                <button
                  type="button"
                  className="flex items-center gap-2 w-full px-4 py-2 text-xs text-rose-500 hover:bg-rose-50/40 transition-colors font-semibold"
                >
