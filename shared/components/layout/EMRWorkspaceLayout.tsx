...
export function EMRWorkspaceLayout({ activeTabId, activeTabName, children }: EMRWorkspaceLayoutProps) {
    ...
    const isDoctorOrNurseFrame = roleKey === 'DOCTOR' || roleKey === 'NURSE';
    const frameRadiusClass = isDoctorOrNurseFrame
        ? 'rounded-tl-[15px] rounded-bl-[48px]'
    ...
    return (
        <div className="flex-1 flex flex-col overflow-hidden bg-transparent pt-6 pb-5 relative">
            ...
            <div
                className={`flex-1 flex flex-col overflow-hidden bg-white ${frameRadiusClass} shadow-[0_4px_20px_-4px_rgba(139,124,246,0.08)] relative z-10`}
            >
