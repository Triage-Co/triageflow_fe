...
export function EMRWorkspaceLayout({ activeTabId, activeTabName, children }: EMRWorkspaceLayoutProps) {
    ...
    const isLabRole = user?.role === 'LAB_TECHNICIAN' || user?.role === 'LAB_STAFF';
    const hideHeaderIds = ['lab', 'reception', 'cashier', 'payment', 'notification', 'notifications', 'setting', 'settings'];
    const showHeader = !isLabRole && !hideHeaderIds.includes(activeTabId);
