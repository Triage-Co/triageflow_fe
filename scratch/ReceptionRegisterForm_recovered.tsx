...
import { symptomTriageService } from '@/modules/reception/services/symptomTriageService';
...
import { REGISTER_DEPARTMENTS, translateSpecialtyDisplayName } from '@/modules/reception/constants/registerDepartments';
...
export function ReceptionRegisterForm() {
    ...
    useEffect(() => {
        ...
        const loadMeta = async () => {
            try {
                ...
                const specialties = await receptionService.getSpecialties(accessToken);
    ...
    const hasValidInterviewToken =
        interviewComplete && Boolean(triageSession.interview_token?.trim());
    const hasBookingPath =
        form.department_id.length > 0 &&
        (form.slot_id.length > 0 || hasValidInterviewToken);
    const step2Valid =
        form.symptoms.trim().length >= 5 &&
        triageSession.is_analyzed &&
        !triageSession.pending_question &&
        hasBookingPath;

    async function lookupPatientByCitizen(citizenId: string) {
        if (!accessToken || citizenId.length < 9) return;

        setIsLookingUp(true);
        setLookupBanner(null);
        try {
            const found = await receptionService.findAccountByCitizenId(citizenId, accessToken);
    ...
    async function handleCccdData(data: CccdScanResult, source: 'qr' | 'image' = 'qr') {
        ...
        await lookupPatientByCitizen(data.citizen_id);
    }
    ...
    function handleNext() {
        ...
        if (step === 1) {
            ...
            if (!triageSession.is_analyzed || triageSession.pending_question) {
                setError('Hãy bấm "Phân tích & gợi ý chuyên khoa" và trả lời đủ các câu hỏi bổ sung.');
                return;
            }
            if (!step2Valid) {
                if (
                    ...
                    setError('Phỏng vấn kết thúc sớm — vui lòng chọn bác sĩ và khung giờ khám.');
                } else {
                    setError('Vui lòng chọn chuyên khoa và khung giờ khám.');
                }
                return;
            }
            setStep(3);
    ...
    const selectedSpecialty = specialties.find((s, i) => {
        const id = s.specialty_id ?? s.specialty_code ?? s.doctor_id ?? String(i);
        return id === form.specialty_id;
    });
    ...
    function handleSubmit() {
        ...
        startTransition(async () => {
            try {
                ...
                } else if (triageSession.interview_token?.trim() && interviewComplete) {
                    const bookingToken = await symptomTriageService.persistDiagnosisForBooking({
                        citizenId: form.citizen_id,
                        dob: form.dob,
                        gender: form.gender,
                        evidence: triageSession.evidence,
                        interviewToken: triageSession.interview_token.trim(),
                    });
                    bookingRes = await receptionService.createBookingRecommend(
                        {
                            patient_id: patientId,
                            interview_token: bookingToken || triageSession.interview_token.trim(),
                        },
                        accessToken,
                    );
                } else {
                ...
                const specialtyLabel = getDepartmentLabel(form.department_id, selectedSpecialty, triageSession);
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        ...
                                        <div className="space-y-1.5">
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => {
                                                    update('citizen_id', e.target.value);
                                                    setLookupBanner(null);
                                                }}
