...
import { Lock, X, Delete, Check, Loader2 } from 'lucide-react';
import { useDisplayPinStore } from '../store/displayPinStore';

interface DisplayPinModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  title?: string;
...
export const DisplayPinModal: React.FC<DisplayPinModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  title = 'Cài đặt thiết bị',
  subtitle = 'Nhập mã PIN quản trị để mở cài đặt màn hình',
}) => {
  const verifyPin = useDisplayPinStore((state) => state.verifyPin);
  ...
  const handleConfirm = async () => {
    if (pinInput.length < 4 || submitting) return;
    setSubmitting(true);
    setErrorMessage('');
    try {
      ...
      const message = err instanceof Error ? err.message : 'Mã PIN không chính xác. Vui lòng thử lại!';
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-md animate-in fade-in duration-200 select-none">
      <div className="bg-white rounded-[32px] border border-slate-100 shadow-2xl w-full max-w-sm p-6 sm:p-7 flex flex-col items-center space-y-5 animate-in zoom-in-95 duration-200">
        ...
        <button
          ...
          disabled={pinInput.length === 0 || submitting}
