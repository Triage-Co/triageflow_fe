        ...
        <div className="lg:col-span-4 bg-[#4F80E1] text-white rounded-[28px] p-6 shadow-xl space-y-5 flex flex-col justify-between">
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              <h3 className="text-2xl font-black">{currentStepItem.subtitle}</h3>
              <p className="text-sm font-bold text-blue-100">{currentStepItem.title}</p>
            </div>

            <div className="space-y-2 text-xs font-bold text-blue-100 pt-2 border-t border-white/20">
              <div className="flex items-center gap-2">
                <MapPin className="w-4 h-4 shrink-0" />
                <span>{currentStepItem.room} - {currentStepItem.location}</span>
              </div>
              ...
              return (
                <div 
                  ...
                  className={cn(
                    ...
                    isCompleted && "bg-neutral-50/80 border-neutral-200/60 opacity-80",
                    isWaiting && "bg-amber-50/40 border-amber-100",
                    step.status === 'pending' && "bg-white border-neutral-100 text-neutral-400"
                  )}
                >
                  <div className="flex items-center gap-3.5">
                    ...
                    <div className={cn(
                      ...
                      isWaiting && "bg-amber-400 text-white",
                      step.status === 'pending' && "bg-neutral-100 text-neutral-400 border border-neutral-200"
                    )}>
                  ...
                  <div className="flex items-center gap-4 text-right">
                    <div className="text-[11px]">
                      ...
                      <span className="text-neutral-400 text-[10px] block">{step.location}</span>
                    </div>
                    ...
                    {step.queueNo && (
                      <div className="text-[11px]">
                        ...
                        <span className="text-neutral-400 text-[10px] block">{step.estimatedWait}</span>
                      </div>
                    )}

                    <span className={cn(
                      "px-3 py-1 rounded-xl text-[10px] font-bold border",
                      isCompleted && "bg-neutral-100 text-neutral-500 border-neutral-200",
                      isInProgress && "bg-[#155DFC] text-white border-[#155DFC]",
                      isWaiting && "bg-amber-100 text-amber-800 border-amber-200",
                      step.status === 'pending' && "bg-neutral-50 text-neutral-400 border-neutral-200"
                    )}>
                      {isCompleted && 'Hoàn thành'}
                      {isInProgress && 'Đang thực hiện'}
                      {isWaiting && 'Đang chờ'}
                      {step.status === 'pending' && 'Chưa thực hiện'}
                    </span>
                  </div>
                </div>
              );
