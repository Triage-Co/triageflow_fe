import type { RegistrationResult, PatientSearchResult } from '@/modules/reception/types/reception.types';
...
// ─── Reissue Ticket (Cấp lại vé) ──────────────────────────────────────────────

function buildReissueTicketHtml(patient: PatientSearchResult): string {
    const now = new Date();
    const pad = (n: number) => n.toString().padStart(2, '0');
    const timeStr = `${pad(now.getHours())}:${pad(now.getMinutes())}:${pad(now.getSeconds())}`;
    const dateStr = `${pad(now.getDate())}/${pad(now.getMonth() + 1)}/${now.getFullYear()}`;
    ...
    const qrPayload = patient.queueId ?? patient.bookingId ?? patient.citizenId ?? '';
    const qrUrl = qrPayload
        ? `https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=${encodeURIComponent(qrPayload)}`
        : '';

    return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Cấp lại vé ${patient.ticketNo ?? ''}</title>
<style>
body{font-family:'Courier New',Courier,monospace,system-ui,sans-serif;padding:8px;max-width:380px;margin:0 auto;color:#000;background:#fff;letter-spacing:-0.2px}
.ticket-container{border:2px solid #000;padding:24px 18px;box-sizing:border-box}
.centered{text-align:center}
...
.system-name{font-size:10px;font-weight:bold;margin-bottom:4px;text-transform:uppercase}
.doc-title{font-size:11px;font-weight:bold;margin-bottom:8px;text-transform:uppercase}
.reissue-badge{display:inline-block;border:2px solid #000;padding:4px 14px;font-size:12px;font-weight:900;text-transform:uppercase;letter-spacing:2px;margin-bottom:6px}
.ticket-box{border:2px solid #000;padding:16px 0;margin:12px auto;width:95%;box-sizing:border-box}
.ticket-box-title{font-size:11px;font-weight:bold;text-transform:uppercase;margin-bottom:6px;letter-spacing:1px}
.ticket-number{font-size:46px;font-weight:900;letter-spacing:2px;line-height:1;margin-bottom:10px}
.priority-box{display:inline-block;border:1px solid #000;padding:4px 10px;font-size:10px;font-weight:bold;text-transform:uppercase}
...
.footer-text{font-size:10px;font-weight:bold;text-transform:uppercase;line-height:1.5;margin-bottom:4px}
.footer-time{font-size:9px;margin-top:12px;font-weight:bold}
.reissue-note{font-size:9px;font-weight:bold;text-transform:uppercase;border:1px dashed #000;padding:4px 8px;margin-top:10px;letter-spacing:0.5px}
</style></head><body>
<div class="ticket-container">
    <div class="centered hospital-name">BỆNH VIỆN ĐA KHOA TRUNG ƯƠNG</div>
    <div class="centered system-name">HỆ THỐNG QUẢN LÝ KHÁM BỆNH TRIAGEFLOW OPD</div>
    <div class="centered"><div class="reissue-badge">★ CẤP LẠI VÉ ★</div></div>
    <div class="centered doc-title">--- PHIẾU ĐĂNG KÝ KHÁM (CẤP LẠI) ---</div>

    <div class="dashed-line"></div>

    <div class="ticket-box">
        <div class="centered ticket-box-title">Số thứ tự khám</div>
        <div class="centered ticket-number">${patient.ticketNo ?? '—'}</div>
        <div class="centered"><div class="priority-box">Đối tượng: ${patient.priority}</div></div>
    </div>

    <div class="dashed-line"></div>

    <table class="info-table">
        <tr><td class="label">HỌ VÀ TÊN:</td><td class="value">${patient.name.toUpperCase()}</td></tr>
        <tr><td class="label">CCCD/CMND:</td><td class="value">${patient.citizenId}</td></tr>
        <tr><td class="label">SỐ ĐIỆN THOẠI:</td><td class="value">${formatPhoneDisplay(patient.phone)}</td></tr>
    </table>

    <div class="dashed-line"></div>

    <table class="info-table">
        <tr><td class="label">KHOA KHÁM:</td><td class="value">${patient.specialty.toUpperCase()}</td></tr>
        <tr><td class="label">TRẠNG THÁI:</td><td class="value">${patient.status}</td></tr>
        ${patient.waitMinutes !== undefined ? `<tr><td class="label">ƯỚC CHỜ:</td><td class="value">${patient.waitMinutes} phút</td></tr>` : ''}
        ${patient.bhyt ? `<tr><td class="label">BHYT:</td><td class="value">${patient.bhyt}</td></tr>` : ''}
    </table>
    ...
    <div class="centered reissue-note">** Đây là vé cấp lại — Số thứ tự không thay đổi **</div>
    <div class="centered footer-time">In lúc: ${printTime}</div>
</div>
<script>window.onload=()=>{window.print();}</script></body></html>`;
}

export function printReissueTicket(patient: PatientSearchResult): void {
    const html = buildReissueTicketHtml(patient);
    const win = window.open('', '_blank', 'width=480,height=720');
    if (!win) return;
    win.document.write(html);
    win.document.close();
...
export function downloadReissueTicketPdf(patient: PatientSearchResult): void {
    const html = buildReissueTicketHtml(patient).replace(
        '<script>window.onload=()=>{window.print();}</script>',
        '<p style="text-align:center;font-size:11px;color:#9ca3af;margin-top:16px">Mở file và chọn In → Lưu dưới dạng PDF</p><script>window.onload=()=>{window.print();}</script>',
    );
    const blob = new Blob([html], { type: 'text/html;charset=utf-8' });
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `cap-lai-ve-${patient.ticketNo ?? patient.citizenId}.html`;
    link.click();
    URL.revokeObjectURL(url);
}
